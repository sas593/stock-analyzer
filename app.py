...
with st.spinner(f"Fetching complete 6-point analysis for {ticker_input}..."):
    try:
        ...
        score_technical = 8 if not hist.empty and price and price > hist['Close'].tail(50).mean() else 4
        score_catalysts = 4
        ...
        if total_score >= 80 and gatekeepers_passed:
            ...
            alloc_pct = "15% - 20% Allocation"
        elif total_score >= 65:
            verdict = "ACCUMULATE 🟡"
            portfolio_role = "Core Quality Compounder (Tier 2)"
            alloc_pct = "5% - 10% Allocation"
        else:
            verdict = "AVOID / WAIT 🔴"
            portfolio_role = "High Risk / Low Growth Probability"
            alloc_pct = "0% Allocation (Watchlist Only)"

        # ================= DISPLAY ALL 6 MODULES =================
        st.header(f"{company_name} ({ticker_input})")
        st.write(f"**Market Cap:** ₹{market_cap:,.0f} Cr | **Live Price:** ₹{price:,.2f}" if price else f"**Market Cap:** ₹{market_cap:,.0f} Cr")

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Overall Score", f"{total_score} / 100")
        m2.metric("5Y Target Multiple", f"{potential_multiple:.1f}x")
        m3.metric("Strategy Verdict", verdict)
        m4.metric("Backtest 5Y CAGR", f"{cagr_5y_actual:+.1f}%")
        ...
        with col_b:
            st.subheader("2. 100-Point Factor Breakdown")
        ...
        with col_c:
            st.subheader("3. Crucial 10x Math Model")
        ...
        with col_d:
            st.subheader("4. Historical 5-Year Backtest")
        ...
        with col_e:
            st.subheader("5. Verdict & Portfolio Allocation")
            st.write(f"• **Strategy Verdict:** {verdict}")
            st.write(f"• **Portfolio Role:** {portfolio_role}")
            st.write(f"• **Target Sizing:** {alloc_pct}")

        with col_f:
            st.subheader("6. Execution Price Milestones")
            if price:
                ...
                st.write("Price data unavailable for milestones.")
